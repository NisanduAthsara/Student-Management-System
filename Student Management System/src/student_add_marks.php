...
if(isset($_GET['name'])){
    ...
    if(isset($_POST['submit_marks'])){
        ...
        $sql_query = "SELECT * FROM group_marks WHERE classno='{$classno}' AND group_name='{$name_url}' AND is_deleted='0' LIMIT 1";
        ...
        if($count == 0){
            ...
            $sql = "INSERT INTO group_marks(classno,group_name,semester,marks,average,rank) VALUES('{$classno}','{$name_url}','{$semester}','{$marks}','{$average}','{$rank}') LIMIT 1";
...
<body>
<div class='back_div'><a href="student_marks.php" class="back_to_reg"><i class="fa fa-arrow-left" aria-hidden="true"></i> Back</a></div>
<!-- <a href='student_add_marks?name={$name_url}.php' class='refresh'><i class='fa fa-refresh' aria-hidden='true'></i>      Refresh</a> -->

<?php
echo "<a href='student_add_marks.php?name={$name_url}' class='refresh new_refre'><i class='fa fa-refresh' aria-hidden='true'></i>      Refresh</a>";
?>
    ...
    <form action="" method="post">
        ...
        <input type="text" name="average" class="sec_formname" placeholder="Average" required=""><br>
        <input type="text" name="rank" class="sec_formname" placeholder="Rank" required=""><br>
    ...
    <?php
        ...
        if(isset($_GET['name'])){
            ...
            if(isset($_POST['search'])){
                $search = $_POST['search'];
                $sql = "SELECT * FROM group_marks WHERE group_name='{$name_url}' AND is_deleted = '0' AND classno = '{$search}' ORDER BY classno";
                ...
                if($count>0){
                    ...
                    echo "<tr><th>Class No.</th><th>Semester</th><th>Marks</th><th>Average</th><th>Rank</th><th>Edit</th><th>Delete</th></tr>";
                    while($res = mysqli_fetch_assoc($result)){
                        echo "<tr><td>{$res['classno']}</td><td>{$res['semester']}</td><td>{$res['marks']}</td><td>{$res['average']}</td><td>{$res['rank']}</td><td><a href='edit_std_marks.php?classno={$res['classno']}&name={$name_url}' class='sec_name_blue sec_name_txt'>Edit</a></td><td><a href='del_std_marks.php?classno={$res['classno']}&name={$name_url}' onclick=\"return confirm('Are you sure?');\" class='sec_name_blue sec_name_txt'>Delete</a></td></tr>";
                    }
                    echo "</table>";
                }else{
                ...
                $sql = "SELECT * FROM group_marks WHERE group_name='{$name_url}' AND is_deleted = '0' ORDER BY classno";
