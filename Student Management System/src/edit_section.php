...
<div class='back_div'><a href="student_marks.php" class="back_to_reg"><i class="fa fa-arrow-left" aria-hidden="true"></i> Back to Student Marks Page</a></div>
...
if(isset($_GET['name'])){
    ...
    $sql = "SELECT * FROM group_names WHERE group_name = '{$name_of_url}' AND is_deleted=0";
    ...
    if(isset($_POST['editbtn'])){
        ...
        $sql = "SELECT * FROM group_names WHERE group_name = '{$new_sec_name}' AND is_deleted=0";
        //unset($new_sec_name);
        ...
        if($count == 0){
            $sql = "UPDATE group_names SET group_name='{$new_sec_name}' WHERE group_name='{$name_of_url}' LIMIT 1";

            $res = mysqli_query($conn,$sql);

            $sql_marks = "UPDATE group_marks SET group_name='{$new_sec_name}' WHERE group_name='{$name_of_url}'";

            $res_marks = mysqli_query($conn,$sql_marks);

            if($res && $res_marks){
                header('Location: student_marks.php?up_sec=success');
                //echo '<p class="info">You have successfully edit name!</p>';
            }
        }else{
            header('Location: student_marks.php?up_sec=failed');
