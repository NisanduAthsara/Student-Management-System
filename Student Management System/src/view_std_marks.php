...
if(isset($_GET['from'])){
    echo "<div class='back_div'><a href=\"student_marks.php\" class=\"back_to_reg\"><i class=\"fa fa-arrow-left\" aria-hidden=\"true\"></i> Back to Student Marks Page</a></div>";
    echo "<a href='view_std_marks.php?from=admin' class='refresh new_refre'><i class='fa fa-refresh' aria-hidden='true'></i>      Refresh</a>";
}else{
...
<?php
    if(isset($_POST['search'])){
        $search = $_POST['search'];
        $sql = "SELECT group_name FROM group_names WHERE (group_name LIKE '%{$search}%') AND is_deleted='0'";
        ...
        if($count==0){
            ...
            $sql = "SELECT * FROM group_names WHERE is_deleted = '0' AND group_name LIKE '%{$search}%' ORDER BY id DESC";
            $res = mysqli_query($conn,$sql);
            if(isset($_GET['from'])){
                if($res){
                    $count = mysqli_num_rows($res);
                    if($count>0){
                        while($result = mysqli_fetch_assoc($res)){
                            echo "<div class='sec_div_view'><a href='view_as_std_marks.php?from=admin&name={$result['group_name']}' class='view_sec_btn'>{$result['group_name']}</a></div><br>";
                        }
                    }
                }
            }else{
                if($res){
                    ...
                    if($count>0){
                        while($result = mysqli_fetch_assoc($res)){
                            echo "<div class='sec_div_view'><a href='view_as_std_marks.php?name={$result['group_name']}' class='view_sec_btn'>{$result['group_name']}</a></div><br>";
                        }
                    }
                }
            }
